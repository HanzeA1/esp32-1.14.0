...
import umqtt.simple2 as simple2
...
class MQTTClient(simple2.MQTTClient):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.sub_to_confirm = {}
        # We store here if there is a connection problem.
    ...
    def is_keepalive(self):
        ...
        """
        time_from__last_cpackage = ticks_diff(
            ticks_ms(), self.last_cpacket) // 1000
    ...
    def add_msg_to_send(self, data):
        ...
        while messages_count >= self.MSG_QUEUE_MAX:
            min_msg_to_confirm = min(
                map(lambda x: x[0] if x else 65535, self.msg_to_confirm.values()), default=0)
    ...
    def publish(self, topic, msg, retain=False, qos=0):
        ...
        if retain:
            ...
            self.msg_to_send[:] = [m for m in self.msg_to_send if not (
                topic == m[0] and retain == m[2])]
        try:
    ...
    def send_queue(self):
        ...
        for data in self.msg_to_send:
            ...
            try:
                ...
                return False
        self.msg_to_send[:] = [
            m for m in self.msg_to_send if m not in msg_to_del]
        del msg_to_del
        ...
        for data in self.sub_to_send:
            ...
            try:
                ...
                return False
        self.sub_to_send[:] = [
            s for s in self.sub_to_send if s not in sub_to_del]
